...
export async function detectEnvironment() {
  ...
  // Check for API keys in env and map to providers
  const providerMap = [
    { env: 'OPENAI_API_KEY', name: 'OpenAI', baseUrl: 'https://api.openai.com/v1', models: ['gpt-4.1-nano', 'gpt-4.1-mini', 'gpt-4.1'] },
    { env: 'ANTHROPIC_API_KEY', name: 'Anthropic', baseUrl: 'https://api.anthropic.com/v1', models: ['claude-haiku-4', 'claude-sonnet-4'] },
    { env: 'GEMINI_API_KEY', name: 'Google Gemini', baseUrl: 'https://generativelanguage.googleapis.com/v1beta/openai', models: ['gemini-2.0-flash'] },
    { env: 'OPENROUTER_API_KEY', name: 'OpenRouter', baseUrl: 'https://openrouter.ai/api/v1', models: ['google/gemini-2.0-flash-exp', 'deepseek/deepseek-chat'] },
    { env: 'XAI_API_KEY', name: 'xAI', baseUrl: 'https://api.x.ai/v1', models: ['grok-3-mini'] },
  ];

  for (const provider of providerMap) {
    if (process.env[provider.env]) {
      detected.availableProviders.push({
        name: provider.name,
        envVar: provider.env,
